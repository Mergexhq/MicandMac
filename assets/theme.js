"use strict";(()=>{var m={header:"#SiteHeader",burger:"#NavToggle",mobileNav:"#MobileNav",navClose:"#NavClose",cartToggle:"#CartToggle",cartDrawer:"#CartDrawer",cartCount:"#CartCount",wishlistCount:".wishlist-count"},h=class{constructor(){this._scrollBound=()=>{};this.header=document.querySelector(m.header),this.burger=document.querySelector(m.burger),this.mobileNav=document.querySelector(m.mobileNav),this.navClose=document.querySelector(m.navClose),this.cartToggle=document.querySelector(m.cartToggle),this.cartDrawer=document.querySelector(m.cartDrawer),this.header&&(this._scrollBound=this._onScroll.bind(this),this._init())}_init(){window.addEventListener("scroll",this._scrollBound,{passive:!0}),this._onScroll(),this.burger?.addEventListener("click",()=>this._openMobileNav()),this.navClose?.addEventListener("click",()=>this._closeMobileNav()),this.cartToggle?.addEventListener("click",()=>this._toggleCart()),l.onClose(()=>{this._closeMobileNav(),this._closeCart()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&(this._closeMobileNav(),this._closeCart())})}_onScroll(){this.header.classList.toggle("is-scrolled",window.scrollY>40)}_openMobileNav(){this.mobileNav?.classList.add("is-open"),this.mobileNav?.setAttribute("aria-hidden","false"),this.burger?.classList.add("is-open"),this.burger?.setAttribute("aria-expanded","true"),l.show(),document.body.style.overflow="hidden"}_closeMobileNav(){this.mobileNav?.classList.remove("is-open"),this.mobileNav?.setAttribute("aria-hidden","true"),this.burger?.classList.remove("is-open"),this.burger?.setAttribute("aria-expanded","false"),document.body.style.overflow="",l.hide()}_toggleCart(){this.cartDrawer?.classList.contains("is-open")?this._closeCart():this._openCart()}_openCart(){this.cartDrawer?.classList.add("is-open"),this.cartDrawer?.setAttribute("aria-hidden","false"),l.show(),document.body.style.overflow="hidden"}_closeCart(){this.cartDrawer?.classList.remove("is-open"),this.cartDrawer?.setAttribute("aria-hidden","true"),document.body.style.overflow="",l.hide()}static updateCartCount(t){let e=document.querySelector("#CartCount");e&&(e.textContent=String(t),e.classList.toggle("cart-count--hidden",t===0),e.classList.remove("bump"),e.offsetWidth,e.classList.add("bump"),setTimeout(()=>e.classList.remove("bump"),400))}static updateWishlistCount(t){document.querySelectorAll(m.wishlistCount).forEach(r=>{r.textContent=t>0?String(t):"",r.classList.toggle("is-hidden",t===0)})}},l=(()=>{let o=document.querySelector("#SiteOverlay"),t=[];return o?.addEventListener("click",()=>t.forEach(e=>e())),{show(){o?.classList.add("is-active"),o?.setAttribute("aria-hidden","false")},hide(){o?.classList.remove("is-active"),o?.setAttribute("aria-hidden","true")},onClose(e){t.push(e)}}})();var H="mm_recent_searches",P=5,p={drawer:"#SearchDrawer",toggle:"#SearchToggle",close:"#SearchDrawerClose",input:"#SearchDrawerInput",clearBtn:"#SearchDrawerClear",results:"#SearchResults",recentWrap:"#SearchRecent",recentList:"#SearchRecentList"},v=class{constructor(){this._debounceTimer=null;this._currentQuery="";this.drawer=document.querySelector(p.drawer),this.toggle=document.querySelector(p.toggle),this.closeBtn=document.querySelector(p.close),this.input=document.querySelector(p.input),this.clearBtn=document.querySelector(p.clearBtn),this.results=document.querySelector(p.results),this.recentWrap=document.querySelector(p.recentWrap),this.recentList=document.querySelector(p.recentList),this.drawer&&this._init()}_init(){this.toggle?.addEventListener("click",()=>this._open()),this.closeBtn?.addEventListener("click",()=>this._close()),l.onClose(()=>this._close()),document.addEventListener("keydown",t=>{t.key==="Escape"&&this._close()}),this.input?.addEventListener("input",t=>{let e=t.target.value.trim();this._onInput(e)}),this.clearBtn?.addEventListener("click",()=>{this.input&&(this.input.value=""),this._onInput(""),this.input?.focus()})}_open(){this.drawer?.classList.add("is-open"),this.drawer?.setAttribute("aria-hidden","false"),l.show(),this._renderRecent(),setTimeout(()=>this.input?.focus(),80)}_close(){this.drawer?.classList.remove("is-open"),this.drawer?.setAttribute("aria-hidden","true"),l.hide()}_onInput(t){if(this._currentQuery=t,this.clearBtn&&(this.clearBtn.hidden=t.length===0),t.length===0){this._clearResults(),this._showRecent(!0);return}this._showRecent(!1),this._debounceTimer&&clearTimeout(this._debounceTimer),this._debounceTimer=setTimeout(()=>{this._currentQuery.length>0&&this._fetchResults(this._currentQuery)},280)}async _fetchResults(t){let e=`/search/suggest.json?q=${encodeURIComponent(t)}&resources[type]=product,page&resources[limit]=5&resources[options][unavailable_products]=last`;try{let s=await(await fetch(e)).json(),n=s?.resources?.results?.products||[],i=s?.resources?.results?.pages||[];if(t!==this._currentQuery)return;if(n.length===0&&i.length===0){this._renderNoResults(t);return}this._renderResults(n,i,t)}catch(r){console.warn("[Search] fetch error",r)}}_renderResults(t,e,r){if(!this.results)return;let s=t.map(i=>`
      <a href="${i.url}" class="search-result-item" data-query="${this._escape(r)}">
        ${i.image?`<img class="search-result-img" src="${i.image}" alt="" width="52" height="52" loading="lazy">`:'<div class="search-result-img"></div>'}
        <div class="search-result-info">
          <p class="search-result-title">${this._highlight(i.title,r)}</p>
          ${i.price!=null?`<p class="search-result-price">\u20B9${(i.price/100).toFixed(0)}</p>`:""}
        </div>
      </a>
    `).join(""),n=e.length?'<div class="search-drawer__label" style="margin-top:var(--space-4)">Pages</div>'+e.map(i=>`
      ...
      ${s}
      ${n}
    `,this.results.querySelectorAll("[data-query]").forEach(i=>{i.addEventListener("click",()=>this._saveRecent(r))})}_renderNoResults(t){this.results&&(this.results.innerHTML=`
      <div class="search-no-results">
        <p>No results for "<strong>${this._escape(t)}</strong>"</p>
        <p style="margin-top:var(--space-2)">Try a different keyword</p>
      </div>
    `)}_clearResults(){this.results&&(this.results.innerHTML="")}_getRecent(){try{return JSON.parse(localStorage.getItem(H)||"[]")}catch{return[]}}_saveRecent(t){let e=this._getRecent().filter(r=>r.toLowerCase()!==t.toLowerCase());e.unshift(t),e.length>P&&e.pop();try{localStorage.setItem(H,JSON.stringify(e))}catch{}}_renderRecent(){let t=this._getRecent();if(this.recentList){if(t.length===0){this._showRecent(!1);return}this._showRecent(!0),this.recentList.innerHTML=t.map(e=>`
      <a href="/search?type=product&q=${encodeURIComponent(e)}" class="search-drawer__recent-tag">
